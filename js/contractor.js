...
const staticTranslations = {
    en: {
        ...
        options: [
         ...
         // THIS SPECIFIC BLOCK IS WHAT FIXES YOUR CRASH:
        newRowCells: [
            "[Edit Phase / Milestone]",
            "[Edit Description Requirement]",
            "[00%]"
        ]
    },
    hi: {
        ...
        options: [
            ...
            { value: "helper", text: "सहायक / सामान्य श्रमिक अनुबंध" }
                ...
                // THIS SPECIFIC BLOCK IS WHAT FIXES YOUR CRASH FOR HINDI MODE:
        newRowCells: [
            "[नया चरण / मील का पत्थर]",
            "[आवश्यकता का विवरण दर्ज करें]",
            "[00%]"
        ]
...
const contractTemplates = {
    en: {
        general: `
            ...
            <div class="table-action-container" contenteditable="false">
            <button id="btn-append-row" class="btn-add-row">+ Add New Row</button>
        </div>
            <table class="payment-table"  id="target-payment-table">
                <thead>
                    <tr>
    ...
    hi: {
        general: `
            ...
            <p>भुगतान को मुख्य डिलिवरेबल्स के आधार पर सुरक्षित रूप से संरचित किया जाएगा जैसा कि नीचे दी गई समय-सारणी में उल्लिखित है:</p>
             <div class="table-action-container" contenteditable="false">
                <button id="btn-append-row" class="btn-add-row">+ नई पंक्ति जोड़ें</button>
            </div>
            <table class="payment-table" id="target-payment-table">
                <thead>
                    <tr>
 ...
 // Standard structural function to listen and append empty text items into row loops safely
    function bindTableAppendAction() {
        const appendButton = document.getElementById("btn-append-row");
        const trackingTable = document.getElementById("target-payment-table");

        if (appendButton && trackingTable) {
            appendButton.addEventListener("click", () => {
                const tbody = trackingTable.querySelector("tbody");
                if (tbody) {
                    const newRow = document.createElement("tr");
                    const cellsData = staticTranslations[currentLang].newRowCells;
    ...
    function updateContractContent() {
        ...
        contentArea.innerHTML = contractTemplates[currentLang][selectedWorker];
          // Always bind fresh event listeners right after modifying internal innerHTML properties
        bindTableAppendAction();
    }
